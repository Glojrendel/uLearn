...
WORDS = ['barbarous','government','godly','glue','glove','throat','harmony','knock','continue','nifty','raise','toothpaste','obeisant','pine','stay','faulty','shave','applaud','blink','spiteful','suggest','paint',
'ancient',
'turkey',
'pocket',
'answer',
...
def hangman():
    ...
    word = choice(WORDS)                                                 # Выбор из списка слов 
    turns = len(HANGMAN)                                                 # Рисунок стадии человечка 
    ...
    while wrong < turns and hidden_letter != word:                       # Если кол-во букв и попыток не равняется загаданному слову 
         os.system('clear')
         print(HANGMAN[wrong])                                           # Печать  теала персонажа
         print ('\nAttempts remain: ' ,counter)
         print('\nYou have already used these letters:\n', used_letter)  
         withoutList = list (hidden_letter)
         print('\nYour hidden word:\n', ' ' .join(withoutList))
         guess = input('\nEnter your letter: ')
...
ans = 'y'                                                                 # Подключить выбор 
while ans == 'y':
    hangman()
    print('Play again? (y/n)')
    ans = input()
else:
    print('As you wish. See you next time. Bye!')
